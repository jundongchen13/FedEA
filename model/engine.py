...
from .tools import aggregateBySimilarity
from .tools import sub_matrix_shift, update_similarity_matrix_neighbor
...
class Engine(object):
    ...
    def federatedTrainSingleBatch(self, model_client, batch_data, optimizer, user_id, iteration):
        ...
        if iteration != 0 and user_id in self.agg_participant_index_map:
            ratings_adapter = model_client.adapter_forward(items, self.server_model_param[self.agg_participant_index_map[user_id]])
    ...
    def aggregateParamsFromClients(self, client_params, **kwargs):
        ...
        # aggregating
        self.server_model_param = aggregateBySimilarity(client_params, kwargs["graph_matrix"], self.config)
        pass
    ...
    def federatedTrainOneRound(self, train_data, item_embeddings, mlp_weights, iteration):
        ...
        for user in participants:
            ...
            for epoch in range(self.config['local_epoch']):
                ...
                for batch_id, batch in enumerate(user_dataloader):
                    ...
                    client_model, client_loss = self.federatedTrainSingleBatch(client_model, batch, optimizer, user, iteration)
        ...
        self.agg_participants = random.sample(list(participant_params.keys()), agg_num)
        self.graph_matrix = torch.ones(len(self.agg_participants), len(self.agg_participants)) / (len(self.agg_participants))
        agg_participants_params = {idx: participant_params[user] for idx, user in enumerate(self.agg_participants)}
        self.agg_client_sample_num = {idx: client_sample_num[user] for idx, user in enumerate(self.agg_participants)}
        self.agg_participant_index_map = {user: idx for idx, user in enumerate(self.agg_participants)}

        self.graph_matrix, self.model_smi = update_similarity_matrix_neighbor(self.graph_matrix,
                                                                            agg_participants_params,
                                                                            self.model.state_dict(),
                                                                            self.agg_client_sample_num,
                                                                            self.config['alpha'],
                                                                            self.config['beta'])
        self.graph_matrix = sub_matrix_shift(self.graph_matrix)
        self.aggregateParamsFromClients(agg_participants_params, client_sample_num=self.agg_client_sample_num,
    ...
    def federatedEvaluate(self, evaluate_data):
        ...
        for user in range(self.config['num_users']):
            ...
            if user in self.agg_participant_index_map:
                user_model.reset_embeddings(self.server_model_param[self.agg_participant_index_map[user]])
